...
export async function POST(request: NextRequest, context: { params: Promise<{ solutionId: string }> }) {
  ...
  try {
    ...
    if (!files.length || files.length > 10) return failure("BRAND_SELECTION_INVALID", "请选择 1–10 个品牌素材。", 422);
    ...
    for (const file of files) {
      if (!file || typeof file.clientKey !== "string" || !file.clientKey || file.clientKey.length > 200 || seen.has(file.clientKey)
        ...
        || !Number.isSafeInteger(file.sizeBytes) || file.sizeBytes <= 0) return failure("BRAND_SELECTION_INVALID", "品牌素材的信息不完整，请重新选择文件。", 422);
      seen.add(file.clientKey);
    }
    const uploads = productSqlite.transaction(() => {
      const insert = productSqlite.prepare("INSERT INTO source_files (id, solution_id, user_id, client_key, category, original_name, declared_mime, size_bytes) VALUES (?, ?, ?, ?, 'brand', ?, ?, ?)");
      return files.map((file) => {
        const fileId = randomUUID();
        insert.run(fileId, solutionId, auth.session.userId, `brand:${file.clientKey}:${fileId}`, file.displayName.trim(), typeof file.declaredMime === "string" ? file.declaredMime.slice(0, 255) : null, file.sizeBytes);
    ...
    return failure("BRAND_SELECTION_FAILED", "暂时无法准备品牌素材上传，请稍后重试。", 500, true);
  }
}
